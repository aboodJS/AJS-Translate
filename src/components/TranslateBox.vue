...
<template>
  <div class="grid justify-center relative border-gray-700 rounded-xl border bg-gray-900  opacity-95 h-90 max-md:w-[95vw]   outline-0 w-lg">
    <select class="grid grid-cols-4 grid-rows-3 text-white rounded-xl relative h-10 max-md:left-8/12 left-10/12 top-4 bg-[#4f5663] outline-0 w-20"  v-model="lang" name="" id=""><option value="ar">arabic</option>
    ...
    <textarea v-model="text" ref="textBox" :disabled="props.editable"   class="indent-2 max-md:w-[95vw] text-white opacity-95 h-64 outline-0 w-lg " name="" id="" maxlength="500"></textarea>
      ...
      }" class="bg-gray-800 flex justify-center items-center p-2.5 cursor-pointer absolute top-74 right-10/12 text-white transition-all hover:bg-white hover:text-black rounded-md"><Icon icon="tabler:copy" width="24" height="24"  /></button>
  </div>
</template>
